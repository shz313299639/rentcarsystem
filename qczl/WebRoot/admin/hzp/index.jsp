...
function top(){
   	form3.action="<%=basePath%>admin/hzp/index.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%>admin/hzp/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/hzp/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/hzp/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/hzp/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/hzp/index.jsp?page="+page;
      form3.submit();
...
<table width='100%' cellspacing='1' cellpadding='3' bgcolor='#CCCCCC' class="tablewidth">
    ...
    <%cb.setEVERYPAGENUM(12);
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from qc  order by id desc",8);
			session.setAttribute("qqq", pagelist1);
			int pageCount = cb.getPageCount(); //得到页数  
			session.setAttribute("busPageCount", pageCount + ""); 
			List pagelist3=(ArrayList)session.getAttribute("qqq"); 
			//List pagelist3=cb.getCom("select * from qc order by id desc",8);
