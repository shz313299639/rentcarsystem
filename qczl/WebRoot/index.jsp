...
<form action="<%=basePath %>searchinfo.jsp" method="post" name="form1" >
    ...
    <option value="汽车名称">汽车名称</option>
           ...
           <input type=button onclick="f1()" value="查询信息">
           </TD>
		</TR>
			</TABLE>
			</form>	
	  ...
	  <TBODY>
        ...
        <MARQUEE onmouseover=if(document.all!=null){this.stop()} onmouseout=if(document.all!=null){this.start()} scrollAmount=1 scrollDelay=1 direction=up height=180>
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              <TBODY>
             <%
              //	List resList=cb.getCom("select qc.id,qc.cm,qc.pic from zc,qc where zc.qcid=qc.id and zt='已审核' ORDER BY zc.qcsj limit 0,5",3);
List resList3=cb.getCom("select id,cm,zccs from qc   ORDER BY zccs desc limit 0,5 ",3);
               	
              	if(!resList3.isEmpty()){
              		for(int res=0;res<resList3.size();res++)
              		{
              			List resList2=(List)resList3.get(res);
              %>
			<TR>
                <TD width="100%">
   <a href="<%=basePath %>hinfo.jsp?id=<%=resList2.get(0).toString()%>"><%=resList2.get(1).toString() %>--<%=resList2.get(2).toString()%>次</A><BR>
				</TD>
			  </TR>
              <%
              		}
...
<!--*******************************热门车型结束*******************************-->
                     ...
                     <TD class=Side vAlign=top align=left width="60%" height=200>
<!--*******************************新闻资讯开始*******************************-->
 <TABLE  class=dragTable cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
 <TBODY>
        <TR>
...
imgUrl[0]='images/s.jpg';
//imgLink[0]='http://www.usth.edu.cn/xwdt/Article/201205/4574.html';
imgtext[0]='宝马炫酷';

imgUrl[1]='images/2.jpg';
//imgLink[1]='http://www.usth.edu.cn/xwdt/Article/201204/4537.html';
imgtext[1]='豪车酷影';

imgUrl[2]='images/3.jpg';
//imgLink[2]='http://www.usth.edu.cn/xwdt/Article/201205/4614.html';
imgtext[2]='越野之最';

imgUrl[3]='images/4.jpg';
//imgLink[3]='http://www.usth.edu.cn/xwdt/Article/201205/4579.html';
imgtext[3]='丰田卡罗拉';
...
<!--*******************************滚动公告开始*******************************-->
	  ...
	  <TBODY>
        ...
        <MARQUEE onmouseover=if(document.all!=null){this.stop()} onmouseout=if(document.all!=null){this.start()} scrollAmount=1 scrollDelay=1 direction=up height=180>
			...
			<TR>
                <TD width="100%">
   <A href="<%=basePath%>gginfo.jsp?id=<%=affList2.get(0).toString()%>" ><%=affList2.get(1).toString() %>--<%=affList2.get(2).toString().substring(0,10) %></A><BR>
				</TD>
			  </TR>
			<%
					}
        ...
        <TR>
          <TD class=middle>
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              ...
              <%
              //	List resList=cb.getCom("select qc.id,qc.cm,qc.pic from zc,qc where zc.qcid=qc.id and zt='已审核' ORDER BY zc.qcsj limit 0,5",3);
List resList=cb.getCom("select id,cm,pic from qc   ORDER BY id desc limit 0,5 ",3);
