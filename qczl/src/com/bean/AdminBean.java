...
public class AdminBean {
	...
	public int editPassword(String username,String oldpwd,String newpwd){
		String sql="select * from admin where username = '"+username+"' and password = '"+oldpwd+"'";//进数据库查询该用户名和密码是否存在
		String sql2="update admin set password = '"+newpwd+"' where username = '"+username+"'";//修改密码SQL语句
		DBO dbo = new DBO();
		dbo.open();
		try{
			rs = dbo.executeQuery(sql);//进行数据库更新操作
			if(rs.next()){
				int i = dbo.executeUpdate(sql2);//进行数据库更新操作
				if(i == 1){
					return Constant.SUCCESS;
				}
				else{
