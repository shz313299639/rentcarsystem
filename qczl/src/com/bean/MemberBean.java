...
//  检查是否重名
    public int checkRegName(String username){
    	String sql = "select * from member where username='"+username+"'";//检查数据库中是否存在重名
    	DBO dbo = new DBO();
		dbo.open();
		try{
...
//	会员修改密码
	public int editPassword(String username,String oldpwd,String newpwd){
		String sql="select * from member where username = '"+username+"' and password = '"+oldpwd+"'";//查询旧密码是否正确
		String sql2="update member set password = '"+newpwd+"' where username = '"+username+"'";//更新新密码
		DBO dbo = new DBO();
		dbo.open();
		try{
			rs = dbo.executeQuery(sql);//执行数据库查询操作
			if(rs.next()){
				int i = dbo.executeUpdate(sql2);//执行数据库更新操作
				if(i == 1){
					return Constant.SUCCESS;
				}
				else{
...
//	会员登陆
	public int memberLogin(String username,String password,String type){
		...
		String sql = "select password from member where username='"+username+"' and type='"+type+"' and ifuse='1' ";//选择数据库中该会员的密码
		...
		try{
			...
			if(rs.next()){//如果有该用户名
				...
				if(str.trim().equals(password)){//如果存在该账户判断该账户数据库密码和输入的密码是否一致
...
//	登陆信息
	public String getLastTimeIP(String username){
		String sql = "select lasttime,lastip from member where username='"+username+"'";//取得会员的最后一次登录时间和IP
		DBO dbo = new DBO();
		dbo.open();
		try{
			rs = dbo.executeQuery(sql);
	...
	//更新登陆信息
	public int upmemberLogin(String username,String lastip ){//更新登录信息
		String sql = "update member set logintimes=logintimes+1,lasttime='"+date+"',lastip='"+lastip+"' where username='"+username+"' ";//更新会员登录的最后一次登录时间和登录次数登录IP等信息
		DBO dbo = new DBO();
		dbo.open();
		try{
	...
	//登陆次数 本次登陆时间
	public String getLogintimes(String username){//取得会员的最后一次登录时间和登录次数
	...
	public int personReg(String username,String password,String type,String realname,String sex,String bir,
		...
		" values('"+username+"','"+password+"','"+type+"','"+date+"','"+off+"','0','"+date+"','"+lastip+"') ";//个人会员注册
		DBO dbo = new DBO();
		dbo.open();
		try{
			int i = dbo.executeUpdate(sql);
			if(i == 1){
				...
				int j = dbo.executeUpdate(sql2);//执行数据库插入操作
				if(j == 1)
					return Constant.SUCCESS;
				else
					return Constant.SYSTEM_ERROR;
	...
	public List getRegInfo(String username){
		String sql = "select * from pmember where mid=(select id from member where username='"+username+"') ";//查询个人会员资料
		DBO dbo = new DBO();
		list = new ArrayList();
		dbo.open();
		try{
			rs = dbo.executeQuery(sql);//将查询结果赋值给LIST
			rs.next();
	...
	public String returnPwd(String username,String question,String answer,String type){
		...
		if(i!=0){
			...
			String sql ="select * from "+type+" where mid='"+i+"' and question='"+question+"' and answer='"+answer+"'";//SQL语句查询用户名和问题、答案是否正确
			DBO dbo=new DBO();
			dbo.open();
			try{
				rs = dbo.executeQuery(sql);
				if(rs.next()){
					String yzm=new CheckCode().getCheckCode()+"123";//正确的话用验证码+123生成最新密码
					String pwd=MD5.MD5(yzm);//生成最新密码用MD5加密
					int k=dbo.executeUpdate("update member set password='"+pwd+"' where username='"+username+"'");//更新数据库的最新密码成功
					if(k==1){
						return yzm;
					}
