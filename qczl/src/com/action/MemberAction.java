...
public class MemberAction extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		else if(method.equals("lostpwd")){//找回密码操作
			String username=request.getParameter("username");//申请用户名变量
			String question=request.getParameter("question");//申请问题变量
			String answer=request.getParameter("answer");//申请答案变量
			String reg_type=request.getParameter("reg_type");//申请账户类型
			String info=memberBean.returnPwd(username, question, answer, reg_type);//执行随机生成最新密码操作通过验证码+123实现
			if(info.trim().equals("error")){
				request.setAttribute("message", "信息错误！");//问题答案错误提示
				request.getRequestDispatcher("login.jsp").forward(request, response);//跳转到登录页面
			}
			else{
				request.setAttribute("message", "您的新密码为"+info+"，请登录后修改！");//提示用户的最新密码是多少
				request.getRequestDispatcher("login.jsp").forward(request, response);
			}
		}
		else{
